!function (Brickrouge) {
	...
	Brickrouge.Tabbable = new Class({
		...
		getPosition: function(tab)
		{
			if (!tab)
			{
				var link = this.nav.querySelector('.nav-link.active')
		...
		activateTab: function(i)
		{
			...
			, links = this.nav.querySelectorAll('.nav-link')
			...
			var activePane = content.querySelector('.active')
	...
	Brickrouge.Widget.TabbableEditor = new Class({
		...
		addTab: function()
		{
			...
			, anchor = new Element('a.nav-link[tabindex="-1"][data-toggle="tab"]', {
				html: '<span data-recieves="text">?</span><span class="close" data-removes="tab">&times;</span>',
				href: '#'
			})
	...
	Brickrouge.register('TabbableEditor', function (element, options) {

		return new Brickrouge.Widget.TabbableEditor(element, options)
