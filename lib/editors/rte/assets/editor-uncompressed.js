...
var blockEls = /^(H[1-6]|HR|P|DIV|ADDRESS|PRE|FORM|TABLE|LI|OL|UL|TD|CAPTION|BLOCKQUOTE|CENTER|DL|DT|DD|SCRIPT|NOSCRIPT|STYLE|ARTICLE|ASIDE|DETAILS|FIGCAPTION|FIGURE|FOOTER|HEADER|HGROUP|NAV|SECTION)$/i;
...
this.MooEditable = new Class({
	...
	options: {
		...
		extraCSS: '',
		externalCSS: '',
		html: '<!DOCTYPE html><html id="{HTMLID}" lang="{LANGUAGE}"><head><meta charset="UTF-8">{BASEHREF}<style>{BASECSS} {EXTRACSS}</style>{EXTERNALCSS}</head><body class="editor-rte"></body></html>',
		rootElement: 'p',
		baseURL: '',
		dimensions: null,
		htmlId: 'wysiwyg'
	...
	initialize: function(el, options){
		...
		this.textarea.addEvent('change', function(ev) {
	...
	attach: function(){
		...
		{
			this.baseHREF += ':' + document.location.port
		}
		...
		if (Browser.safari || Browser.chrome){
			...
			if (Element.get(el, 'tag') == 'img'){
				...
				if (this.options.baseURL){
					...
					console.log('src: %s (%s)', el.getProperty('src'), el.getProperty('src').match('/https?\:\/\//'))

					if (!el.getProperty('src').match('/https?\:\/\//')){
	...
	setContent: function(content){
		...
		content = this.manageSrcAttribute(content)
	...
	manageSrcAttribute: function(content)
	{
		var baseHREF = this.href

		return content.replace(/src="([^"]+)/gi, function(match, src) {
...
;!function() {
	...
	MooEditable.UI.Button = new Class({
		...
		render: function()
		{
			var shortcut = this.options.shortcut ? ' ( ' + CMD + '+' + this.options.shortcut.toUpperCase() + ' )' : ''
			, name = this.options.name
			, icon = this.options.icon || name
			, text = this.options.title || name
			, title = text + shortcut

			this.el = new Element('button', {

				'class': 'mooeditable-ui-button ' + this.options['class'],
				title: title,
				tabindex: -1,
				html: '<span class="icon-' + icon + ' button-icon"></span><span class="button-text">' + text + '</span>',
				events: {
	...
	({
		...
		open: function()
		{
			...
			if (this.popover)
			{
				...
				this.popover.adjust.setValue(this.getValidNode())
				this.popover.show()
			}
			else
			{
		...
		{
			var AdjustPopover = require('icybee/adjust-popover')
			var popover = this.popover = new AdjustPopover(popElement, {
			...
			adjust.setValue(this.getValidNode())
